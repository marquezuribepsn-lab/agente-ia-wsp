...
// (OpenRouter expone modelos muy distintos entre sí en ese aspecto).
...
async function callModel(
  ...
  // Con modelos ":free" de OpenRouter, una respuesta de error/rate-limit a
  // veces llega con HTTP 200 pero sin "choices" (en vez de tirar una
  // excepción). Chequeamos explícito para dar un error claro en logs en vez
  // de un TypeError críptico.
  if (!Array.isArray(completion.choices) || completion.choices.length === 0) {
    console.error(
      "[openrouter] Respuesta sin 'choices':",
      JSON.stringify(completion)
    );
    throw new Error(
      "El LLM no devolvió 'choices' en la respuesta (puede ser un límite de rate del modelo :free)."
    );
  }
